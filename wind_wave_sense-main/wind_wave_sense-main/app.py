    ...
    try:
        ...
        if user:
            if check_password_hash(user['password'], password):
                ...
                return redirect(url_for('map_page'))  # Redirect to the user map page
            else:
                flash("Invalid password for user!", "error")
                return redirect(url_for('login'))

        # If not found in users, check the admins table for admin users
        cursor.execute('SELECT * FROM admin WHERE username = %s', (username,))
        admin = cursor.fetchone()

        if admin:
            # If admin is found in the admin table, check the password
        ...
        # If neither user nor admin found, flash an error message
